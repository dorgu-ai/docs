...
spec:
  policies:
    selfHealing:
      enabled: true
      mode: observe          # observe | propose | auto-approve
      trustLevel: 2          # 0-5
...
<Warning>
  **Today, `mode` and `trustLevel` are descriptive, not enforced.** They are recorded on the ClusterPersona and fed to the AI planner as context, and the operator auto-creates its default persona with `mode: observe`. But no code path gates proposal on the mode, and **auto-approve graduation is not implemented** — `spec.approval.autoApproveRule` exists in the CRD and controllers ignore it. Every remediation requires human approval, whatever the mode says. `maxRemediationsPerHour` and `excludeNamespaces` *are* enforced.
</Warning>
