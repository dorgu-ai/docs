...
<Note>
  **Auto-approve graduation is not implemented.** The `ClusterPersona` carries `policies.selfHealing.mode` (`observe`, `propose`, `auto-approve`) and `trustLevel` (0–5), and `RemediationAction` carries `approval.autoApproveRule` — but controllers ignore all three when deciding whether approval is needed. Every remediation waits for a human, regardless of what the mode says. `maxRemediationsPerHour` and `excludeNamespaces` on that same policy *are* enforced.
</Note>
