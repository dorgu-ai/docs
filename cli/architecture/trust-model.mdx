...
| Owned by Helm, ArgoCD, Flux | Yes | Yes, shaped for that owner | No |
| `managedBy: kustomize` | Yes | Yes, shaped for that owner | No. Set only from an [opt-in marker](/ownership-model#the-kustomize-limitation); a plain overlay reads as `unmanaged`. |
