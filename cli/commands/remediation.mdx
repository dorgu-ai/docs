...
NAMESPACE   NAME                PHASE    TYPE            PLAN          STEPS  SEVERITY  CONFIDENCE  PERSONA     AGE
...
| SEVERITY | Legacy field — see the note below |
...
<Note>
  **SEVERITY renders blank.** `severity` is a legacy CLI-side field that the current operator's `RemediationAction` CRD does not set, so the column is empty and `--severity` matches nothing against objects created by the operator. Filter by `--phase` or read severity from the linked incident with [`dorgu incidents describe`](/cli/commands/incidents) instead.
</Note>
...
| `--next` | bool | `false` | Pick a pending remediation automatically instead of naming one |
...
<Note>
  `--next` ranks pending remediations by severity and takes the top one. Because the operator does not set `severity` today, all pending actions rank equally — treat `--next` as "one of the pending remediations", and name the resource explicitly when it matters.
</Note>
