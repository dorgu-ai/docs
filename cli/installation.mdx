...
Download `dorgu_windows_amd64.zip` (or `_arm64`) from the [latest release](https://github.com/dorgu-ai/dorgu/releases/latest), extract it, and put `dorgu.exe` somewhere on your `PATH`. The pinned form is `dorgu_0.12.0_windows_amd64.zip`.
...
```bash
go install github.com/dorgu-ai/dorgu/cmd/dorgu@v0.12.0
```

<Note>
  If the literal above has fallen behind, [Versions](#versions) is the statement to trust on this page, and [CLI releases](https://github.com/dorgu-ai/dorgu/releases) is the statement to trust over both. `@latest` always resolves to the newest.
</Note>
...
```
dorgu version v0.12.0
Build: 0d863c6 @ 2026-08-26T22:40:37Z
```
