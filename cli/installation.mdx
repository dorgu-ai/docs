...
Download `dorgu_windows_amd64.zip` (or `_arm64`) from the [latest release](https://github.com/dorgu-ai/dorgu/releases/latest), extract it, and put `dorgu.exe` somewhere on your `PATH`. The pinned form is `dorgu_0.9.0_windows_amd64.zip`.
...
```bash
go install github.com/dorgu-ai/dorgu/cmd/dorgu@v0.9.0
```

<Note>
  `v0.9.0` is the version this page is pinned to. Check [CLI releases](https://github.com/dorgu-ai/dorgu/releases) for the newest, or use `@latest` to always get it.
</Note>
