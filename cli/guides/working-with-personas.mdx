...
The starting point decides the command.

| You have | Use | Reads |
|----------|-----|-------|
| Local source with a `Dockerfile` or `docker-compose.yml` | `dorgu persona generate` / `dorgu persona apply` | Your working tree |
| Deployments already running on the cluster | [`dorgu persona import`](/cli/commands/persona#persona-import) | The live Deployment spec |

<Tip>
  **On an existing cluster, start with import.** `generate` and `apply` need local source, which you do not have for the twenty services already running. Import reads the Deployments instead:

  ```bash
  dorgu health                                # which Deployments have no persona
  dorgu persona import -n apps --all          # read the personas it would create
  dorgu persona import -n apps --all --apply  # create them
  ```

  Requires CLI v0.9.0 or newer. Full walkthrough: [brownfield quickstart](/cli/quickstart#brownfield-a-cluster-that-already-has-apps).
</Tip>
...
<Steps>
  <Step title="Generate">
    ...
    On a cluster that already runs the app, use [`dorgu persona import`](/cli/commands/persona#persona-import) instead: it needs no local source and no Dockerfile.
  </Step>
