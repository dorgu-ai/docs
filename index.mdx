...
<Note>
  ...
  **And the CLI will not write workloads you have not handed it.** It detects who owns each Deployment and refuses to patch one that Helm, ArgoCD, or Flux reconciles, because that patch would break the owner's next apply. Those three run controllers and stamp what they reconcile; [kustomize does neither](/ownership-model#the-kustomize-limitation), and Dorgu says so rather than claiming otherwise. See [the ownership model](/ownership-model).
</Note>
...
- **Respects your deployment tool**: Dorgu detects who owns each workload and refuses to patch a Helm, ArgoCD, or Flux-managed Deployment. It tells you what to change in your source of truth instead, so your next `helm upgrade` still works. And where it does patch, it removes its own field-manager entry afterwards, so it leaves no ownership footprint for your next apply to fight
...
- **BYO key, and AI off by default**: Anthropic only, operator-side. The deterministic path is the default and the current recommendation, and it heals on its own. [Why](/operator/configuration/ai-setup#why-it-is-off-and-what-that-costs-you)
